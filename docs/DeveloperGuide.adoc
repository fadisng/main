...
Methods 3-4 are only for sorting tasks wheres method 5 is for sort spending.
...
These are the stages shown in figure 9. Stages are similar to sort spending as well.
...
. Obtains list of tasks from model and sorts it.
...
* **Alternative 1 (current choice):** Store the data a in list.
